...
public function saveprofile(Request $request)
    {
       ...
       $request->validate([
        ...
        if(!empty(DB::table('users')->where('email',$request->profile_job_join_email)->value('email'))){        

         $pass=array([
            'email'=>$request->profile_job_join_email,
            'password'=>Hash::make($randome_pass),
         ]);
         DB::table('users')->where('email',$request->profile_job_join_email)->delete(); 
       ...
       //   dd($subsidiaries_logo);
         ...
         Mail::mailer('it')->to($request->profile_job_join_email)->send(new PasswordSend($mailData));
